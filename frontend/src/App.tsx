...
import { LayoutDashboard, Map as MapIcon, ListTodo, Calendar, Settings, ShieldAlert, LogOut } from 'lucide-react';
      ...
      <div className="w-96 bg-white p-8 rounded-xl shadow-lg border border-slate-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} className="mt-1 w-full p-2 border border-slate-300 rounded-md focus:ring-blue-500 focus:border-blue-500" required />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700">Password</label>
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} className="mt-1 w-full p-2 border border-slate-300 rounded-md focus:ring-blue-500 focus:border-blue-500" required />
          </div>
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const tasksRes = await axios.get(`${API_URL}/maintenance/tasks`, { headers });
        const plansRes = await axios.get(`${API_URL}/plans/optimized`, { headers });
        const stationsRes = await axios.get(`${API_URL}/stations`, { headers });
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
          <div className="text-slate-500 text-sm font-medium">Optimized Blocks</div>
          <div className="text-2xl font-bold text-blue-600 mt-1">{new Set(plans.map(p => p.block_id)).size || 0} Scheduled</div>
        </div>
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
          <div className="text-slate-500 text-sm font-medium">AI Optimization</div>
          <div className="text-2xl font-bold text-indigo-600 mt-1">CP-SAT Active</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100 bg-slate-50">
            <h2 className="font-semibold text-slate-800">Optimized Block Schedule</h2>
          </div>
          <div className="p-5">
             {loading ? <p>Loading...</p> : (
                ...
                {Array.from(new Set(plans.map(p => p.block_id))).slice(0, 3).map((block_id: any) => {
                  ...
                  return (
                    <div key={block_id} className="border border-indigo-100 rounded-lg p-4 bg-indigo-50/50">
                      ...
                      <div className="text-xs text-slate-500 mb-3">Tasks bundled: {blockTasks.length}</div>
